...
export default function TransactionCard({ tx }: { tx: Transaction }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-800 hover:bg-gray-900/50 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5">
          <span className="text-gray-400 text-xs">#{tx.invoice_id}</span>
          {tx.branch && <span className="text-gray-500 text-xs">{tx.branch}</span>}
      ...
      <div className="w-24 text-right flex-shrink-0">
        ...
        {tx.cashback_amount != null && tx.cashback_amount > 0 && (
          <p className="text-emerald-400 text-xs">+${Number(tx.cashback_amount).toFixed(2)} CB</p>
        )}
      </div>
